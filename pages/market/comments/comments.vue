		...
		<scroll-view class="trends_square" :scroll-y="true" @scrolltolower="getRemainComments">
			...
			<view class="trends_card">
				...
				<view class="trends_card_head">
					<!-- 用户头像 -->
					<view class="avator">
						<u-avatar :src="trade_info.user.profile_pic" mode="square" size="mini" @click="toPastport(item.user)"></u-avatar>
					</view>
					<!-- 用户名字 -->
					<view class="name">
						<!-- {{item.user_name}} -->
						<text>{{trade_info.user.nickname}}</text>
					</view>
					<!-- 标题 -->
					<view class="title">
						<text>{{trade_info.title}}</text>
					</view>
					<!-- 操作 -->
					<view class="operate">
				...
				<view class="trends_card_content">
					...
					<view class="content_img" >
						<view class="content_img1" v-for="(item1,index1) in trade_pic" :key="index1">
				...
				<view class="trends_card_bottom">
					...
					<view class="like">
						...
						<text>{{trade_info.thumbs_up}}</text>
					</view>
					<!-- 评论 -->
					<view class="comment">
						<u-icon name="weixin-fill" size="36"></u-icon>
						<text>{{trade_info.comment_num}}</text>
					</view>
					<!-- 分享 -->
					<view class="share">
		...
		methods: {
			...
			async getComments(){
				...
				const result = await this.$myRequest({
					method: 'GET',
					url: '/trade_comments/?trade=' + this.trade_info.id,
					header: head,
				})
				// console.log(result)
			...
			async submitComment(){
				...
				const result = await this.$myRequest({
					...
					url: '/trade_comments/',
					header: head,
					data:{comment:this.post_comment_form_content,trade:this.trade_info.id}
				})
				console.log(result)
				// 清空
